...
class PiicoDev_Radio:
	def __init__(self,bus=_A,freq=_A,sda=_A,scl=_A,address=_BASE_ADDRESS,id=_A,radio_address=1,channel=0,suppress_warnings=False):
		try:
			if compat_ind>=1:0
		...
		self._write_int(_REG_RFM69_NODE_ID,radio_address);self._write_int(_REG_RFM69_NETWORK_ID,channel);self.rssi=0;self.type=0;self.message='';self.key='';self.value=_A;self.source_radio_address=0
		try:
	...
	def send_payload(self,payload):
		payload_list=[payload[i:i+_MAXIMUM_I2C_SIZE-1]for i in range(0,len(payload),_MAXIMUM_I2C_SIZE-1)];self._write_int(_REG_PAYLOAD_LENGTH,len(payload));print('Payload Length:'+str(len(payload)));sleep_ms(5)
		for i in range(len(payload_list)):self._write(_REG_PAYLOAD,payload_list[i]);sleep_ms(5);print('payload'+str(payload_list[i]))
		self._write_int(_REG_PAYLOAD_GO,1)
	def receive_payload(self):
		payload_length=0;payload=bytes(0)
		if self._payload_new==1:
			sleep_ms(100);payload_length=self._read_int(_REG_PAYLOAD_LENGTH)+2;unprocessed_payload_length=payload_length;sleep_ms(5);number_of_chunks=int(truncate(payload_length/_MAXIMUM_I2C_SIZE))+1
	...
	def destination_radio_address(self,value):debug('Setting destination radio address to '+str(value)+'.');self._write_int(_REG_RFM69_TO_NODE_ID,value)
	...
	def receive(self):
		...
		if payload_length!=0:
			payload_bytes=bytes(payload);print(payload_bytes);self.rssi=-int.from_bytes(payload_bytes[:1],_B);self.source_radio_address=int.from_bytes(payload_bytes[1:2],_B);self.type=int.from_bytes(payload_bytes[2:3],_B)
			try:
				if self.type==1:self.key=str(payload_bytes[8:],_C);self.value=unpack('>i',payload_bytes[3:7])[0]
				if self.type==2:self.key=str(payload_bytes[8:],_C);self.value=unpack('>f',payload_bytes[3:7])[0]
				if self.type==3:self.message=str(payload_bytes[3:],_C)
			except:print('* error parsing payload')
			return True
		return False
	def send(self,message_string,value=_A,address=0):
		A='s';debug('sending stuff');self.destination_radio_address=address
		if isinstance(value,int):type=1;message_string=message_string[:_MAXIMUM_PAYLOAD_LENGTH-6];format_characters='>BiB'+str(len(message_string))+A;data=pack(format_characters,type,value,len(message_string),bytes(message_string,_C))
		if isinstance(value,float):type=2;message_string=message_string[:_MAXIMUM_PAYLOAD_LENGTH-6];format_characters='>BfB'+str(len(message_string))+A;data=pack(format_characters,type,value,len(message_string),bytes(message_string,_C))
		if value is _A:type=3;message_string=message_string[:_MAXIMUM_PAYLOAD_LENGTH-2];format_characters='>BB'+str(len(message_string))+A;data=pack(format_characters,type,len(message_string),bytes(message_string,_C))
